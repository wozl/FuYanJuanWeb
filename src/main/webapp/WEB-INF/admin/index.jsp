...
<body data-type="widgets">
    ...
    function Article(){
		$.ajax({
			type:'post',
			url:'${pageContext.request.contextPath}/Admin/ArticleAll.do',
			data:{},
			success:function(data){
				...
				if(list.length>0){
					 ...
					 for(var i=0;i<list.length;i++){
						...
						'<a href="##" onclick="redact(this)"><i class="am-icon-pencil"></i> 评阅</a>'+
    ...
    function Fuzzy_search(obj){//模糊搜索
		...
		$.ajax({
				type:'post',
				url:"${pageContext.request.contextPath}/Admin/SearchByTitle.do",
				data:{"title":title},
				success:function(data){
					console.log(data);
					...
					if(data.length==0){
						...
						for(var i=0;i<data.length;i++){
							$("#options").append('<option value="'+data[i].title+'" onclick="get(this);">'+data[i].title+'</option>');
						}
					}
				},
				error:function(){
    ...
    function search(){
		...
		$.ajax({
			type:'post',
			url:'${pageContext.request.contextPath}/Admin/TitleInfo.do',
			data:{"title":value},
			success:function(data){
					console.log(data);
						document.getElementById("TableContent").innerHTML="";
